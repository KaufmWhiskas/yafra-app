...
Deno.test(
  'createSearchFetcher() makes a POST request to Google Places API with correct headers and body',
  async () => {
...
Deno.test(
  'createSearchFetcher() maps Google API response to Prediction[] format',
  async () => {
    const originalFetch = globalThis.fetch;
...
Deno.test(
  'createSearchFetcher() includes locationBias when location is provided',
  async () => {
    const originalFetch = globalThis.fetch;
