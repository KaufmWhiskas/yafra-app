...
Deno.test(
  'createGoogleFetcher() should call Google Places API with correct headers to enforce $0 SKU',
  async () => {
...
Deno.test(
  'createGoogleFetcher() should correctly parse Google API response into RestaurantRecords',
  async () => {
    const originalFetch = globalThis.fetch;
...
Deno.test(
  'createGoogleFetcher() should convert BoundingBox to a circle locationRestriction',
  async () => {
    const originalFetch = globalThis.fetch;
